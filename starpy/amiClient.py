...
from ccc.asterisk.models import AMIAction

log = logging.getLogger('ccc')
log.debug('testing logging through asterisk.amiClient')

class AMIClient(basic.LineOnlyReceiver):
    ...
    def __init__( self, *args, **named ):
        """Initialise the AMIClient, arguments are ignored"""
        self.messageCache = []
    ...
    def sendMessage( self, message, responseCallback=None ):
        ...
        amiAction = AMIAction()
        if 'ext_key' in message:
            amiAction.ext_key = message.pop('ext_key')
        amiAction.action = message.get('action','')
        amiAction.save()
        message['actionid'] = '%s-%s'%(id(self),str(amiAction.id))
    ...
    def originate( 
        ...
        data=None, variable={}, async=False, attempt_id=None
    ):
        ...
        message = dict([(k,v) for (k,v) in {
            ...
            'data':data, 'variable':variable, 'async':str(async), 'ext_key':str(attempt_id)
...
class AMIFactory(protocol.ClientFactory):
    ...
    """
    protocol = AMIClient
    def __init__(self, username, secret, events, connectionLostHandler=None):
    ...
    def clientConnectionLost( self, connector, reason):
        log.info('connection lost inside ami client')
...
class AMIManager(threading.Thread):
    def __init__(self, usr, secret, host, port, keys, callback, timeout=30.0):
        self.usr = usr
        self.secret = secret
        self.host = host
        self.port = port
    ...
    def get(self, key):
        return self.amis.get(key,None)

    def onStart(self):
        for key in self.keys:
            log.info( 'Creating ami client for %s' % key)
            def onConnect(key):
                def storeConnection(ami):
                    log.info( 'Created ami client for %s' % key)
                    self.amis[key] = ami
                    self.callback(key, ami)
                return storeConnection
            def onError(key):
                def errorConnecting(reason):
                    log.error( 'Error creating ami client for %s' % key)
                return errorConnecting
            events = 'off'
            if key == 'events':
                events = 'on'
            f = AMIFactory(self.usr, self.secret, events)
            df = f.login(self.host, self.port, self.timeout)
            df.addCallback(onConnect(key))
