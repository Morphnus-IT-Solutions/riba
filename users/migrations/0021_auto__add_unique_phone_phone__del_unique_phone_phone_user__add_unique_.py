...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding unique constraint on 'Phone', fields ['phone']
        db.create_unique('users_phone', ['phone'])

        # Adding index on 'Phone', fields ['type']
        db.create_index('users_phone', ['type'])

        # Removing unique constraint on 'Phone', fields ['phone', 'user']
        db.delete_unique('users_phone', ['phone', 'user_id'])

        # Adding unique constraint on 'Email', fields ['email']
        db.create_unique('users_email', ['email'])

        # Adding index on 'Email', fields ['type']
        db.create_index('users_email', ['type'])

        # Removing unique constraint on 'Email', fields ['user', 'email']
        db.delete_unique('users_email', ['user_id', 'email'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Phone', fields ['phone']
        db.delete_unique('users_phone', ['phone'])

        # Removing index on 'Phone', fields ['type']
        db.delete_index('users_phone', ['type'])

        # Adding unique constraint on 'Phone', fields ['phone', 'user']
        db.create_unique('users_phone', ['phone', 'user_id'])

        # Removing unique constraint on 'Email', fields ['email']
        db.delete_unique('users_email', ['email'])

        # Removing index on 'Email', fields ['type']
        db.delete_index('users_email', ['type'])

        # Adding unique constraint on 'Email', fields ['user', 'email']
        db.create_unique('users_email', ['user_id', 'email'])
    ...
    models = {
        ...
        'users.dailysubscription': {
            ...
            'email_alert': ('django.db.models.fields.BooleanField', [], {'default': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'newsletter': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['users.NewsLetter']"}),
            'sms_alert': ('django.db.models.fields.BooleanField', [], {'default': 'True', 'blank': 'True'}),
