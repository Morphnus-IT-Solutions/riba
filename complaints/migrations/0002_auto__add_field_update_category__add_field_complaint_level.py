...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Update.category'
        db.add_column('complaints_update', 'category', self.gf('django.db.models.fields.CharField')(max_length=50, null=True, blank=True), keep_default=False)

        # Adding field 'Complaint.level'
        db.add_column('complaints_complaint', 'level', self.gf('django.db.models.fields.CharField')(db_index=True, max_length=15, null=True, blank=True), keep_default=False)

        # Adding M2M table for field products on 'Complaint'
        db.create_table('complaints_complaint_products', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('complaint', models.ForeignKey(orm['complaints.complaint'], null=False)),
            ('product', models.ForeignKey(orm['catalog.product'], null=False))
        ))
        db.create_unique('complaints_complaint_products', ['complaint_id', 'product_id'])


    def backwards(self, orm):
        
        # Deleting field 'Update.category'
        db.delete_column('complaints_update', 'category')

        # Deleting field 'Complaint.level'
        db.delete_column('complaints_complaint', 'level')

        # Removing M2M table for field products on 'Complaint'
        db.delete_table('complaints_complaint_products')
