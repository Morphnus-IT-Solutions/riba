...
def render_common(template_name, obj, filename, version):
    ...
    context['request'] = obj.get('request', None)
...
def compressed_css(parser, token):
    try:
        tag_name, name, request = token.split_contents()
    except ValueError:
    ...
    return CompressedCSSNode(name, request)
...
def compressed_js(parser, token):
    ...
    return CompressedJSNode(name, request)
