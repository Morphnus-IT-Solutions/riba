...
def create_request(payment_attempt, request):
    ...
    data['catalogName'] = CATALOG_NAME.get(request.client.client.name, 'chaupaati')
    ...
    data['returnUrl'] = 'https://%s/orders/process_payment' % domain
    ...
    if payment_attempt.order.client_id == 1:
