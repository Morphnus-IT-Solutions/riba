...
class HoliiFeed(Feed):
    ...
    def get_feature_group(self):
        try:
            get_feature_group = FeatureGroup.objects.get(name = 'Holii Specifications', product_type = ProductType.objects.get(type = 'HandBags'))
        except FeatureGroup.DoesNotExist:
            get_feature_group = FeatureGroup(name = 'Holii Specifications', product_type = ProductType.objects.get(type = 'HandBags'))
    ...
    def get_mapped_sku_type(self, sku_type):
        try:
            ...
            mapping = SkuTypeProductTypeMapping(account = self.config['ACCOUNT'], sku_type = sku_type, product_type = ProductType.objects.get(type = sku_type))
            mapping.save()
    ...
    def parse(self, sync, *args, **kwargs):
        ...
        try:
            get_product_type = ProductType.objects.get(type = 'HandBags')
        except ProductType.DoesNotExist:
            get_product_type = ProductType(type = 'HandBags')
        ...
        for row_num in range(1,s.nrows):
            row = s.row(row_num)
            brand = row[heading_map['category 1']].value
            parent_category = row[heading_map['category 3']].value
        ...
        for row_num in range(1,s.nrows):
            ...
            for variants in range(row_num,(row_num+int(temp_row[heading_map['variant']].value))):
                ...
                if list_price == offer_price and today in [30]:
                    offer_price = int(0.8*float(list_price))
                    #offer_price = Decimal(str(round(str(((Decimal(str(0.8))*Decimal(list_price)))))))
                print "O P::",type(offer_price)
                ...
                try:
                    ...
                    for infile in listing:
                        if infile != 'Thumbs.db':
                            image_urls.append('http://dev.holii.in:8000/media/images/pd/%s/%s' % (str(int(row[heading_map['image url']].value)), infile))
                except Exception, e:
                ...
                try:
                    holii_price_list = PriceList.objects.get(name='Holii')
                except PriceList.DoesNotExist:
                    holii_price_list = PriceList(name='Holii')
