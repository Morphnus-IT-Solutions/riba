...
class EzoneOnlineFeed(Feed):
    config = {
            'ACCOUNT': 'ezoneonline',
            'URL': 'http://feeds.futurebazaar.com:%s/catalog/1000028/categories' % settings.FEED_SYNC_PORT_NUMBER,
            'TYPE': 'JSON'
            }
    ...
    def get_child(self,category,products,level):
        ...
        for product in category['products']:
            ...
            for sku in product['skus']:
                ...
                extracted_data['category'] = Category.objects.get(client__id=7,ext_id=category['category_id'])
                data['cleaned_data']['title'] = extracted_data['title']
                data['cleaned_data']['category'] = Category.objects.get(client__id=7,ext_id=category['category_id'])
                data['cleaned_data']['external_product_id'] = extracted_data['product_id']
                slo = sku.get('ship_local_only','')
                if slo in [None, "None"]:
                    slo = False
                otc = sku.get('otc','')
                if otc in [None, "None"]:
                    otc = False
                data['cleaned_data']['otc'] = otc
                data['cleaned_data']['ship_local_only'] = slo
                ...
                timed_ezone_price_list = []
                ezone_timed_price_lists = []
                if sku.get('future_slots',None):
                    try:
                        timed_ezone_price_list = PriceList.objects.get(name='Timed Ezone Price')
                    except PriceList.DoesNotExist:
                        timed_ezone_price_list = PriceList(name='Timed Ezone Price')
                        timed_ezone_price_list.save()

                    for future_slot in sku['future_slots']:
                        ezone_timed_price_lists.append({
                            'price_list' : timed_ezone_price_list,
                ...
                # price list priorities. first ezone, then anonymous 
                list_price_priority = ['plist1810002', 'plist130002']
                sale_price_priority = ['plist1810003', 'plist130003']
                ...
                #Populating Pricing.price table for ezone pricelists
                #First, Ezone prices
                ezone_price_list = None

                ezone_list_price = price_dict.get('plist1810002',None)
                ezone_offer_price = price_dict.get('plist1810003',None)
                ezone_anonymous_list_price = price_dict.get('plist130002',None)
                ezone_anonymous_offer_price = price_dict.get('plist130003',None)

                try:
                    ezone_price_list = PriceList.objects.get(name='Ezone')
                except PriceList.DoesNotExist:
                    ezone_price_list = PriceList(name='Ezone')
                    ezone_price_list.save()

                applicable_price_lists.append(ezone_price_list)

                if ezone_offer_price:
                    if sku.get('slot_price', None):
                        ezone_offer_price = Decimal(sku['slot_price'])

                    if ezone_list_price:
                        if Decimal(ezone_list_price) < Decimal(ezone_offer_price):
                            if ezone_anonymous_list_price and (Decimal(ezone_anonymous_list_price) >= Decimal(ezone_offer_price)):
                                ezone_list_price = ezone_anonymous_list_price
                            else:
                                ezone_list_price = ezone_offer_price
                    else:
                        if ezone_anonymous_list_price and (Decimal(ezone_anonymous_list_price) >= Decimal(ezone_offer_price)):
                            ezone_list_price = ezone_anonymous_list_price
                        else:
                    ...
                    price_lists.append({
                        'price_list':ezone_price_list,
                        'list_price':ezone_list_price,
                        'offer_price':ezone_offer_price})
                
                #Second, Anonymous Ezone prices
                ezone_anonymous_price_list = None

                try:
                    ezone_anonymous_price_list = PriceList.objects.get(name='Anonymous Ezone Pricelist')
                except PriceList.DoesNotExist:
                    ezone_anonymous_price_list = PriceList(name='Anonymous Ezone Pricelist')
                    ezone_anonymous_price_list.save()

                applicable_price_lists.append(ezone_anonymous_price_list)

                if ezone_anonymous_offer_price:
                    if not ezone_offer_price:
                        if sku.get('slot_price', None):
                            ezone_anonymous_offer_price = Decimal(sku['slot_price'])

                    if ezone_anonymous_list_price:
                        if Decimal(ezone_anonymous_list_price) < Decimal(ezone_anonymous_offer_price):
                            ezone_anonymous_list_price = ezone_anonymous_offer_price
                    else:
                        ezone_anonymous_list_price = ezone_anonymous_offer_price
                    
                    price_lists.append({
                        'price_list':ezone_anonymous_price_list,
                        'list_price':ezone_anonymous_list_price,
                        'offer_price':ezone_anonymous_offer_price})
                
                data['cleaned_data']['price_lists'] = price_lists
                data['cleaned_data']['fb_timed_price_lists'] = ezone_timed_price_lists
...
if __name__ == '__main__':
    feed = EzoneOnlineFeed(False, False, False)
    feed.sync()
