...
class MicromaxFeed(Feed):
    ...
    def parse(self, sync, *args, **kwargs):
        ...
        url = 'http://192.168.91.101:8080/listing/select?indent=on&version=2.2&q=userId%3A186841+AND+isActive%3Atrue&start=0&rows=500&fl=*&qt=standard&wt=json&explainOther=&hl.fl='
        ...
        for product in prods:
            ...
            if not data['sku']:
                data['cleaned_data']['sku'] = 'micromax%s' % count
                count += 1
            else:
                data['cleaned_data']['sku'] = data['sku']
            make = 'micromax'
            data['cleaned_data']['brand'] = Brand.objects.get(name=make)
            data['cleaned_data']['category'] = Category.objects.get(name='Mobiles')
