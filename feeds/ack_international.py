...
class ACKFeed(Feed):
    ...
    def parse(self, sync, *args, **kwargs):
        ...
        for product in xmldoc.xpath("/xml/node"):
            ...
            #data['cleaned_data']['shipping_duration'] = '7-10 Working Days'
            data['cleaned_data']['list_price'] = Decimal(self.get_text(data['List_price']).replace(',','').replace('Rs','')) 
            data['cleaned_data']['offer_price'] = Decimal(self.get_text(data['Sell_price']).replace('Rs','').replace(',',''))
