...
class RobinageFeed(Feed):
    ...
    def parse(self, sync, *args, **kwargs):
        ...
        #next gen publishing url = 'http://192.168.91.101:8080/listing/select?indent=on&version=2.2&q=make%3A%22nextgen+publishing%22+AND+isActive%3Atrue+AND+sku%3A[*+TO+*]&start=0&rows=100&fl=*%2Cscore&qt=standard&wt=json&explainOther=&hl.fl='

        url ='http://192.168.91.101:8080/listing/select?indent=on&version=2.2&q=make%3A%22outlook+group%22+AND+isActive%3Atrue&start=0&rows=100&fl=*%2Cscore&qt=standard&wt=json&explainOther=&hl.fl='
        ...
        for product in prods:
            ...
            data['cleaned_data']['brand'] = Brand.objects.get(name='outlook group')
